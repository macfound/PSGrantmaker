...
Every call to the Fluxx API requires an authentication token. This call returns an object containing a token (access_token), token type (token_type), and the number of seconds before the token expires (expires_in). This should be the first call made before any others. The access_token attribute is used as the -BearerToken parameter in all of the other calls.
